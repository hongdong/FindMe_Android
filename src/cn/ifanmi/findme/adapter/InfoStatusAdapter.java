...
public class InfoStatusAdapter extends BaseAdapter {

	private Context context;
	private List<InfoStatus> infoStatuses;
	
	public InfoStatusAdapter(Context context, List<InfoStatus> infoStatuses) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (null == convertView) {
			...
			case NormalString.StatusViewType.NO_PICTURE:
				...
				holder.img_isread = (ImageView)convertView.findViewById(R.id.img_status_info_isread);
		...
		Status status = infoStatuses.get(position).getStatus();
		...
		String isRead = infoStatuses.get(position).getIsRead();
		if ("0".equals(isRead)) {
			holder.img_isread.setVisibility(View.VISIBLE);
		} else if ("1".equals(isRead)) {
			holder.img_isread.setVisibility(View.GONE);
		}
	...
	public int getItemViewType(int position) {
		String[] photos = infoStatuses.get(position).getStatus().getPhotos().split(",");
		if (1 == photos.length) {
			if ("".equals(photos[0])) {
